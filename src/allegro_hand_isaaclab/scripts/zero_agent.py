...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Stop after this many environment steps. Defaults to None (run until the visualizer is closed).",
)
...
import v5_sense_isaaclab_pipeline.tasks  # noqa: F401
        ...
        while True:
            ...
            if sim.visualizers:
                ...
                if not any(v.is_running() and not v.is_closed for v in sim.visualizers):
                    break
            elif args_cli.max_steps is None:
                raise RuntimeError(
                    "No visualizer is open and '--max_steps' was not given, so the agent would run forever."
                    " Pass '--max_steps N' for headless runs."
                )
