...
enum eMotionType
{
	...
	eMotionType_ENVELOP,			///< envelop grasp(powerful grasp)
...
private:
	...
	static const int MA_WINDOW_SIZE = 3;
	double _q_buffer[NOF][NOJ][3];	///< buffer for moving average filter (10 samples)
	int _q_buffer_index[NOF][NOJ];		///< current buffer index for each joint
	int _q_buffer_count[NOF][NOJ];		///< number of samples in buffer (for initial phase)
	...
	double _kp[NOF][NOJ];				///< proportional control gain for each joint (used in Joint PD)
	double _kd[NOF][NOJ];				///< derivative control gain for each joint (used in Joint PD)
	double _kp1[NOF][NOJ];				///< proportional control gain for each joint (used in Pose PD)
	double _kd1[NOF][NOJ];				///< derivative control gain for each joint (used in Pose PD)
	double _ki1[NOF][NOJ];
